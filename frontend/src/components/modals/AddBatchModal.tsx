...
export const AddBatchModal: React.FC<AddBatchModalProps> = ({ isOpen, onClose, courses, staff, editingBatch, onSave, onArchive }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-slate-950/55 p-0 backdrop-blur-sm sm:items-center sm:p-4">
      <div ref={dialogRef} tabIndex={-1} role="dialog" aria-modal="true" aria-labelledby="add-batch-title" className="relative max-h-[92dvh] w-full max-w-lg overflow-y-auto rounded-t-3xl border border-brand-200/50 bg-white p-4 shadow-2xl dark:bg-slate-900 sm:rounded-2xl sm:p-6 space-y-4">
        ...
        <div className="sticky top-0 z-30 flex justify-end pointer-events-none -mb-10 sm:-mb-12">
          <div className="pointer-events-auto flex items-center bg-white/90 dark:bg-slate-900/90 backdrop-blur-md p-1.5 rounded-2xl shadow-lg border border-slate-200/80 dark:border-slate-700/80">
            <button type="button" onClick={onClose} aria-label="Close add batch" title="Close"
        ...
        <div className="flex justify-between items-center border-b border-slate-100 dark:border-slate-800 pb-3 pt-1">
          <h3 id="add-batch-title" className="font-heading text-xl font-bold text-slate-900 dark:text-white pr-12">
