...
import { FeeReceipt, OrgSettings } from '../../types';
import { useDialogLifecycle } from './useDialogLifecycle';

export function FeeReceiptModal({ isOpen, onClose, receipt, settings }: { isOpen: boolean; onClose: () => void; receipt: FeeReceipt | null; settings: OrgSettings }) {
      ...
      <article className="fee-receipt-print rounded-2xl border border-slate-200 bg-white p-5 text-slate-900 shadow-sm sm:p-7">
        <header className="flex items-start justify-between gap-4 border-b border-dashed border-slate-300 pb-5">
          <div className="flex min-w-0 items-center gap-3">
            {settings.receipt.showLogo && <div className="flex h-12 w-12 shrink-0 items-center justify-center overflow-hidden rounded-xl bg-brand-500 text-lg font-extrabold text-white">{settings.logoUrl ? <img src={settings.logoUrl} alt={settings.name} className="h-full w-full object-cover" /> : settings.name.charAt(0)}</div>}
            <div className="min-w-0"><h3 className="truncate text-base font-extrabold">{settings.name}</h3><p className="mt-0.5 text-[11px] leading-5 text-slate-500">{settings.receipt.address || settings.type}{settings.receipt.phone && <><br />{settings.receipt.phone}</>}{settings.receipt.email && <><br />{settings.receipt.email}</>}</p></div>
          </div>
        ...
        <div className="grid grid-cols-2 gap-4 border-b border-dashed border-slate-300 py-5 text-xs sm:grid-cols-3">
          <ReceiptDetail label="Student" value={receipt.studentName} /><ReceiptDetail label="Student ID" value={receipt.studentNumber} /><ReceiptDetail label="Course" value={receipt.course} />
          <ReceiptDetail label="Payment date" value={date.toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' })} /><ReceiptDetail label="Payment method" value={receipt.paymentMethod} /><ReceiptDetail label="Status" value="Paid" tone="success" />
        </div>
        <div className="my-5 flex items-end justify-between rounded-xl bg-brand-50 px-4 py-4"><div><p className="text-[10px] font-bold uppercase tracking-wider text-slate-500">Amount received</p><p className="mt-1 text-xs text-slate-500">Monthly fee payment</p></div><p className="text-2xl font-extrabold tabular-nums text-brand-700">₹{receipt.amount.toLocaleString('en-IN')}</p></div>
        <footer className="pt-2 text-center"><p className="text-xs text-slate-500">{settings.receipt.footer}</p>{settings.receipt.showSignature && <div className="ml-auto mt-10 w-36 border-t border-slate-400 pt-1 text-[10px] text-slate-500">Authorized signature</div>}</footer>
      </article>
