...
export const WhatsAppModal: React.FC<WhatsAppModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-slate-950/55 p-0 backdrop-blur-sm sm:items-center sm:p-4">
      <div ref={dialogRef} tabIndex={-1} role="dialog" aria-modal="true" aria-labelledby="whatsapp-modal-title" className="max-h-[92dvh] w-full max-w-md overflow-y-auto rounded-t-3xl border border-brand-200/50 bg-white p-4 shadow-2xl dark:bg-slate-900 sm:rounded-2xl sm:p-6 space-y-4">
        <div className="flex justify-between items-center border-b border-[#f3faf7] dark:border-[#1e293b] pb-3">
          <h3 id="whatsapp-modal-title" className="font-heading text-lg sm:text-xl font-bold text-[#0b1c30] dark:text-[#f8fafc] flex items-center gap-2">
          ...
          <button type="button" onClick={onClose} aria-label="Close WhatsApp message" className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl text-[#565e74] hover:bg-slate-100 hover:text-[#0b1c30] dark:hover:bg-slate-800">
