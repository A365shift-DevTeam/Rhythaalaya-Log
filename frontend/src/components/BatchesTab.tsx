  ...
  return (
    <div className="space-y-4 sm:space-y-6 md:space-y-8 pb-12">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-5">
        {/* Courses Section */}
        <section className="premium-card rounded-2xl sm:rounded-3xl overflow-hidden">
          <div className="p-4 sm:p-5 flex items-center justify-between border-b border-slate-100 dark:border-slate-800">
            <div>
              <h3 className="font-heading text-base sm:text-lg font-extrabold text-slate-900 dark:text-white">Courses</h3>
              <p className="text-[11px] text-slate-500">{courses.length} active programs</p>
            </div>
            {canManage && (
              <button
                type="button"
                onClick={onOpenAddCourse}
                className="btn-brand min-h-10 rounded-xl px-3 text-xs font-bold flex items-center gap-1 active:scale-95"
              >
          ...
          <div className="divide-y divide-slate-100 dark:divide-slate-800 max-h-72 overflow-y-auto">
            ...
            {courses.map((course) => (
              <button
                ...
                disabled={!canManage}
                className="w-full flex items-center justify-between gap-3 p-3.5 sm:p-4 text-left hover:bg-slate-50 dark:hover:bg-slate-800/50 disabled:cursor-default disabled:hover:bg-transparent transition-colors"
              >
                <div className="min-w-0">
                  <div className="text-xs sm:text-sm font-bold text-slate-900 dark:text-white truncate">{course.name}</div>
        ...
        <section className="premium-card rounded-2xl sm:rounded-3xl overflow-hidden">
          <div className="p-4 sm:p-5 flex items-center justify-between border-b border-slate-100 dark:border-slate-800">
            <div>
              <h3 className="font-heading text-base sm:text-lg font-extrabold text-slate-900 dark:text-white">Staff & mentors</h3>
              <p className="text-[11px] text-slate-500">{staff.length} instructors</p>
            </div>
            {canManage && (
              <button
                type="button"
          ...
          <div className="divide-y divide-slate-100 dark:divide-slate-800 max-h-72 overflow-y-auto">
            ...
            {staff.map((member) => (
              ...
              >
                <div className="min-w-0">
                  <div className="text-xs sm:text-sm font-bold text-slate-900 dark:text-white truncate">{member.name}</div>
      ...
      {/* All Batches Section */}
      <section className="premium-card rounded-2xl sm:rounded-3xl overflow-hidden">
        <div className="p-4 sm:p-5 md:p-6 border-b border-slate-100 dark:border-slate-800">
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3 sm:gap-4">
            <div>
              <h3 className="font-heading text-lg sm:text-xl font-extrabold text-slate-900 dark:text-white">All batches</h3>
              <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">{filteredBatches.length} of {batches.length} batches shown</p>
            </div>
            ...
            <div className="grid grid-cols-1 sm:flex sm:flex-wrap items-center gap-2">
              {canManage && (
                <button
                  ...
                  className="btn-brand min-h-11 rounded-xl px-4 text-xs font-bold flex items-center justify-center gap-1.5 shrink-0 active:scale-95"
                >
              ...
              <div className="relative flex-1 sm:w-64">
                ...
                <input
                  ...
                  className="w-full rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 pl-10 pr-9 py-2.5 min-h-11 text-xs text-slate-900 dark:text-white outline-none focus:border-brand-400 focus:ring-2 focus:ring-brand-500/10"
                />
                {search && (
                  <button
                    type="button"
              ...
              <select
                ...
                className="rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 px-3 py-2.5 min-h-11 text-xs font-semibold text-slate-700 dark:text-slate-200 outline-none focus:border-brand-400 cursor-pointer"
              >
        ...
        {filteredBatches.length === 0 ? (
          <div className="text-center py-12 px-4">
            ...
            <h4 className="font-heading font-bold text-slate-900 dark:text-white mt-3">
    ...
    <article className="premium-card-interactive group p-4 sm:p-5 flex flex-col justify-between">
      <div>
        <div className="flex items-start justify-between gap-3">
          <div className="w-10 h-10 rounded-xl bg-brand-100 dark:bg-brand-900 text-brand-700 dark:text-brand-300 flex items-center justify-center shrink-0">
            <span className="material-symbols-outlined text-[20px]">calendar_view_week</span>
          </div>
          <div className="flex items-center gap-1.5">
            <span
              className={`rounded-full px-2.5 py-0.5 text-[10px] font-bold inline-flex items-center gap-1.5 ${
                batch.isActive
                  ? 'bg-emerald-50 dark:bg-emerald-950/60 text-emerald-700 dark:text-emerald-300'
                  : 'bg-slate-100 dark:bg-slate-800 text-slate-500'
              }`}
            >
              <span className={`w-1.5 h-1.5 rounded-full ${batch.isActive ? 'bg-emerald-500' : 'bg-slate-400'}`} />
            ...
            {canManage && (
              <button
                ...
                className="w-8 h-8 inline-flex items-center justify-center rounded-lg text-slate-400 hover:text-brand-600 hover:bg-brand-50 dark:hover:bg-slate-800 transition-colors"
              >
        ...
        <div className="mt-3">
          ...
          <p className="text-xs font-semibold text-brand-600 dark:text-brand-400 mt-0.5">{batch.courseName}</p>
        </div>
      ...
      <div className="mt-4 pt-3 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between">
        <div className="text-[11px] text-slate-500 font-medium">Enrollment</div>
        <div className="inline-flex items-center gap-1.5 rounded-full bg-blue-50 dark:bg-blue-950/50 text-blue-700 dark:text-blue-300 px-2.5 py-0.5 text-xs font-bold">
  ...
  return (
    <div className="flex items-start gap-2">
      ...
      <div className="min-w-0">
        ...
        <div className="text-xs font-semibold text-slate-700 dark:text-slate-200 break-words">{value}</div>
      </div>
    </div>
  );
}
...
function BatchStat({
  ...
  return (
    <div className="premium-card p-3 sm:p-4 flex flex-col justify-between">
      <div className={'w-8 h-8 sm:w-9 sm:h-9 rounded-xl flex items-center justify-center ' + colors[color]}>
        <span className="material-symbols-outlined text-[18px] sm:text-[20px]">{icon}</span>
      </div>
      <div className="text-xl sm:text-2xl font-black text-slate-900 dark:text-white mt-2 sm:mt-3 tabular-nums">{value}</div>
      <div className="text-[10px] sm:text-[11px] font-semibold text-slate-500 dark:text-slate-400 mt-0.5 truncate">{label}</div>
    </div>
  );
}
